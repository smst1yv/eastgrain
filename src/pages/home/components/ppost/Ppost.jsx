...
const Ppost = () => {
  ...
  const [newsTitleKey, setNewsTitleKey] = useState([])
  const newstitlekey = 'news_title_key';

  useEffect(() => {
    const fetchNewsTitleKey = async  () => {
      try {
        const responseKey = await axios.get(`${config.apiUrl}translation/${newstitlekey}?lang=${await storedLanguage()}`)
        setNewsTitleKey(responseKey.data)
      } catch (error) {
  ...
  const [newsButtonKey, setNewsButtonKey] = useState([])
  const newsbuttonkey = 'news_button_key';

  useEffect(() => {
    const fetchNewsButtonKey = async  () => {
      try {
        const responseKey = await axios.get(`${config.apiUrl}translation/${newsbuttonkey}?lang=${await storedLanguage()}`)
        setNewsButtonKey(responseKey.data)
      } catch (error) {
        console.error('Error', error);
      }
    }
  ...
  return (
    <>
        <div className="section news landing">
      <h5 className="h5-dark">{newsTitleKey.title}</h5>
      <div className={`${styles['news-container']}`}>
                    ...
                    <div id='w-node-_62c79e8d-c8f4-e88d-723f-a6a78017a3f9-383ae574' className={`${styles['news-list-wrapper']} ${styles.rest} ${styles['w-dyn-list']}`}>
                        ...
                        <div role='list' className={`${styles['news-list']} ${styles['w-dyn-items']}`}>
                            {blogData && blogData.slice(1 , 4).map((item, index) => (
      ...
      <a href="/news" className="button dark w-button">{newsButtonKey.title}</a>
        </div>
